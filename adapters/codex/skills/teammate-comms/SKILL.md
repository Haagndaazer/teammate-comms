...
| `teammate_whoami` | `verbose?` | Your registration state, identity, team, harness, comms dir, and your own profile (diagnostics). `verbose:true` adds a read-only **doctor** report — comms root, per-agent heartbeat liveness, sub-stream file sizes, unread counts, leftover lock dirs, and the legacy-root migration state. |
...
- **Spawned subagent** (a lead invoked it via the `spawn_agent` tool): has no
...
`$TEAMMATE_COMMS_DIR`, else `~/.teammate-comms` — so agents across different projects
(and different harnesses) share one space and can message each other. A pre-0.16.0
`~/.claude/TeammateComms` tree is moved to the new root automatically by the first
server that starts while no old-version agent is live; until then the legacy root keeps
being used so a rolling upgrade never splits the team (`teammate_whoami verbose`
reports the state). For per-project isolation, set `$TEAMMATE_COMMS_DIR` (or pass
`comms_dir`) to the project dir. Two instances must share the same root to message each
other; `teammate_whoami` reports the resolved root.
...
Everything above is harness-neutral. Facts specific to Codex:
...
- Server stderr is in the harness's session log.
